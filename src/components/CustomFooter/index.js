...
const CustomFooter = ({state, descriptors, navigation}) => {
  ...
  //to select updated cartcount data from redux//
  ...
  const getCartProducts = async () => {
    await firestore()
      ...
      .get()
      .then(snapshot => {
       console.warn(snapshot.size);
       //data passing to redux to update count//
      ...
      {state.routes.map((route, index) => {
        ...
        return (
          <TouchableOpacity
            ...
            style={responsiveStyle.touchConainer}>
...
//state.routesil aa drawerilulla routes name ane undavuka//
